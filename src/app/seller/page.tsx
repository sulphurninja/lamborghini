...
export default function SellerDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (data.success) {
        // Filter to show only users created by current seller
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              <Card className="p-3">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-blue-600">{currentUser?.credits || 0}</p>
                  </div>
                </div>
              </Card>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card>
            <CardContent className="p-6">
              ...
              <div className="text-sm text-gray-500">Total Users Created</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-6">
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>User Creation Cost</CardTitle>
            <CardDescription>
              Cost deducted from your credits for each user creation
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Dialog open={createDialog} onOpenChange={setCreateDialog}>
                ...
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Create a new user. {currentUser?.pricing?.user_creation_cost || 0} credits will be deducted.
                    </DialogDescription>
                  </DialogHeader>
                  <form onSubmit={handleCreate} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Enter unique user key"
                        required
                      />
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="plan_expiry">Plan Expiry Date</Label>
                      <Input
                        id="plan_expiry"
                        type="date"
                    ...
                    <div className="p-4 bg-yellow-50 rounded-lg">
                      <p className="text-sm text-yellow-800">
                        <strong>Cost:</strong> {currentUser?.pricing?.user_creation_cost || 0} credits
                      </p>
                      <p className="text-sm text-yellow-800">
                        <strong>Remaining after creation:</strong> {' '}
                        {(currentUser?.credits || 0) - (currentUser?.pricing?.user_creation_cost || 0)} credits
                      </p>
                    </div>
                    ...
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button 
                        type="submit" 
                        disabled={loading || (currentUser?.credits || 0) < (currentUser?.pricing?.user_creation_cost || 0)}
                      >
                  ...
                  {users.map((user) => (
                    <TableRow key={user.id}>
                      <TableCell className="font-medium">
                        {user.key.length > 30 ? user.key.substring(0, 30) + '...' : user.key}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <span className={user.remainingDays < 0 ? 'text-red-600 font-bold' : 'text-green-600'}>
                          {user.remainingDays}
                        </span>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={editDialog} onOpenChange={setEditDialog}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <form onSubmit={handleEdit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edit-key">User Key</Label>
              <Input
                id="edit-key"
                value={formData.key}
                onChange={(e) => setFormData({...formData, key: e.target.value})}
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-plan-expiry">Plan Expiry Date</Label>
              <Input
                id="edit-plan-expiry"
                type="date"
                value={formData.plan_expiry}
